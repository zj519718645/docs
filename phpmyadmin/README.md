...
```console
$ docker run --name myadmin -d --link mysql_db_server:db -p 8080:80 winamd64/phpmyadmin
```
...
```console
$ docker run --name myadmin -d -e PMA_HOST=dbhost -p 8080:80 winamd64/phpmyadmin
```
...
```console
$ docker run --name myadmin -d -e PMA_ARBITRARY=1 -p 8080:80 winamd64/phpmyadmin
```
...
```console
$ docker run --name myadmin -d --link mysql_db_server:db -p 8080:80 -v /some/local/directory/config.user.inc.php:/etc/phpmyadmin/config.user.inc.php winamd64/phpmyadmin
```
